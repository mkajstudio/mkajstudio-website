/**
 * MKAJ ADMIN ENGINE - V35.3 (CARD VIEW FULL INFO + DATE GROUPING)
 */
...
window.recalculatePrice = function() {
    ...
    if (themeType === 'couple') {
        ...
        if (totalHead > 4) {
            alert(`⚠️ Tema "${themeSelect.value}" terhad kepada MAX 4 Pax. Bilangan peserta dilaraskan.`);
            adultInput.value = 2; kidInput.value = 0; adults = 2; kids = 0; totalHead = 2;
        }
    } else {
...
function renderCards(data) {
    ...
    data.forEach(b => {
        ...
        card.className = `bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col gap-4 card-${statusKey}`;
        card.innerHTML = `
            <div class="flex justify-between items-start">
                ...
                <div class="text-right">
                    ...
                    <p class="text-[8px] font-black text-slate-400 uppercase mt-1 tracking-widest">${b.Photographer || 'TBA'}</p>
                </div>
            </div>
...
function getActionButtons(b, statusKey, payType) {
    ...
    if (statusKey !== 'canceled') {
        ...
        else btns += `<button onclick="handleN8N('${b.OrderID}', 'resit')" class="w-8 h-8 rounded-lg bg-indigo-100 text-indigo-600 hover:bg-indigo-600 hover:text-white transition flex items-center justify-center" title="Resit"><i class="fas fa-receipt text-xs"></i></button>`;
    }
...
window.applyFilters = function() {
    ...
    let filtered = allBookings.filter(b => {
        ...
        if (fD === 'today') mD = (bD === todayStr);
        else if (fD === 'tomorrow') {
            const tObj = new Date(); tObj.setDate(tObj.getDate()+1);
            mD = (bD === getLocalISO(tObj));
        }
        else if (fD === 'week') {
            const curr = new Date();
            const first = curr.getDate() - curr.getDay();
            const startW = getLocalISO(new Date(new Date().setDate(first)));
            const endW = getLocalISO(new Date(new Date().setDate(first + 6)));
            mD = (bD >= startW && bD <= endW);
        }
        else if (fD === 'custom') {
            const st = document.getElementById('startDate').value;
            const en = document.getElementById('endDate').value;
            if (st && en) mD = (bD >= st && bD <= en); else if (st) mD = (bD === st);
        }
        return mS && mT && mSt && mD;
    });
...
window.editCustomer = function(id) {
    ...
    document.getElementById('form-extraTime').checked = /extra\s*time/i.test(b.AddOns || "");
    ...
    for (let i = 0; i < mSelect.options.length; i++) {
        ...
        if (s !== "tiada" && clean(b.AddOns).includes(s)) { mSelect.selectedIndex = i; break; }
    }
...
function populateDropdowns() {
    ...
    tSelects.forEach(select => { if (select) { select.innerHTML = select.id === 'filterTheme' ? '<option value="all">Semua Tema</option>' : ''; for (const [key, val] of Object.entries(rayaThemesDetail)) { let opt = document.createElement('option'); opt.value = val.title; opt.innerText = val.title; select.appendChild(opt); } } });
